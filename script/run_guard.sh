#!/bin/bash
# sudo ./run_guard.sh /path/to/gclod.json tufinim.hopto.org/gm

if [ "$#" -ne 2 ]
then
    echo "Usage: sudo $0 <gcloud-key-file> <IP or DNS of Guard Manager>"
    exit 1
fi
...
gcloud docker -- pull eu.gcr.io/tufin-container-security/tufin-guard:build216
if [ "$?" -ne 0 ]; then
